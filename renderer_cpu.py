...
class AVRRender(nn.Module):
    ...
    def forward(self, rays_o, position_tx, direction_tx=None):
        ...
        d_vals = torch.linspace(0., 1., self.n_samples) * (self.far - self.near) + self.near  # scale t with near and far
        ...
        zero_mask_tail = torch.where((torch.arange(signal.size(-1)-1, 0-1, -1).unsqueeze(0) - shift_samples.unsqueeze(1))>0, 1, 0)
        signal = signal.to("cpu") * zero_mask_tail
        ...
        range_tensor = torch.arange(signal.size(-1)) # [N_lenseq]
        ...
        ideal_dis2rx = torch.arange(0, signal.size(-1)*2.5, device='cpu') / self.fs * self.speed
        ...
        phase_shift = torch.exp(-1j*2*np.pi/signal.size(-1)*torch.arange(0, signal.size(-1)//2+1).unsqueeze(0)*pts2rx_idx.unsqueeze(1))
...
def ray_directions(n_azi, n_ele, random_azi=True):
    ...
    # Azimuth direction
    azi_ray = torch.linspace(0, np.pi*2, n_azi+1)[:-1]
    azi_randadd = (np.pi*2 / n_azi) * torch.rand(n_azi) # Randomlly add an angle shift
    azi_ray = azi_ray + azi_randadd if random_azi else azi_ray

    # Elevation direction
    ele_ray = torch.linspace(0, 1, n_ele+2)[1:-1] + (0.5 / n_ele) * torch.rand(n_ele) * 0 
    ...
    dir = torch.cat((dir, torch.tensor([[0,0,1], [0,0,-1]])), dim=0) # [n_azi * n_ele + 2, 3]
...
def acoustic_render(attn, signal, r_vals):
    ...
    att_i = torch.cumprod(torch.cat([torch.ones((alpha[...,:1].shape)), 1.-alpha + 1e-6], -1), -1)[...,:-1]
