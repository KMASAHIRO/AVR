...
def tensor_info(tensor, name="tensor"):
    if torch.is_tensor(tensor):
        ...
        print(f"[Tensor Info] {name}: shape={tuple(tensor.shape)}, dtype={tensor.dtype}, size={size_mb:.2f} MB")

def dump_memory_snapshot(tag=""):
    snapshot = torch.cuda.memory_snapshot()
    with open(f"cuda_memory_snapshot_{tag}.txt", "w") as f:
        pprint.pprint(snapshot, stream=f)
...
def log_gpu_memory(tag=""):
    if torch.cuda.is_available():
        ...
        # nvidia-smi コマンドでより詳細な使用状況を確認
        #try:
        #    print(f"[{tag}] nvidia-smi:")
        ...
        while self.current_iteration <= self.total_iterations:
            with tqdm(total=len(self.train_iter), desc=f"Iteration {self.current_iteration}/{self.total_iterations}") as pbar:
                for train_batch in self.train_iter:
                    ...
                    dump_memory_snapshot("before_renderer")
                    ...
                    if self.dataset_type == "RAF":
                        ...
                        tensor_info(position_rx, "position_rx")
                        tensor_info(position_tx, "position_tx")
                        log_gpu_memory(f"Iter {self.current_iteration} - After batch")
                        
                        pred_sig = self.renderer(position_rx.cuda(), position_tx.cuda())
                        tensor_info(pred_sig, "pred_sig")
                    
                    log_gpu_memory(f"Iter {self.current_iteration} - After renderer")
                    dump_memory_snapshot("after_renderer")
                    ...
                    log_gpu_memory(f"Iter {self.current_iteration} - After total loss")

                    self.optimizer.zero_grad()
                    total_loss.backward()

                    log_gpu_memory(f"Iter {self.current_iteration} - After backward")
                    ...
                    log_gpu_memory(f"Iter {self.current_iteration} - After optimizer.step")
